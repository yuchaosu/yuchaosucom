...
- Isolate the corresponding part of grammar
- Force the parser to recognize high precedence subexpressions first.

Example:
Original Grammar
<img src = "contextfreeoriginal.png" >
String: $x-2*y$
The precedence of multiply and minus is radom. But in reality, multiply has higher precedence. To change this, we need to rewrite the grammar.
Changed Grammar
<img src = "contextfreeprecedence.png" >
...
### Top-down parsing
LL(1) Lookahead 1 token and leftmost
...
#### Left Recursion
A grammar is left recursive if $$ such that
Top-down parser is left most recursion, so it can not handle the left recursion.
So, we will try to make the left recursion grammar to right recursion grammar.
...
Fie -> alpha Fie | alpha | epsilon

Hint: When have left recursion, it maybe not direct. Solve the direct recursion first, then replace the non-terminal in the future expression.

#### Backtrack

When parsing the string, the parser will try all possible expression on the string. If it encounter a problem, it will go back to upper branch to try next available expression.
To have better efficiency and performance we want to eliminate the backtrack in parsing process.
Solution: Look ahead, you may look ahead k tokens, but in common we only look 1 token ahead.
